...
$form->setWidget('restore_template_from', 'select', array('values' => array_merge(array('select' => $this->lang('select one')),CMSUsers::$frontend_templates)));
$form->setWidget('default_template_for', 'select', array('values' => array_merge(array('select' => $this->lang('select one')),CMSUsers::$frontend_templates)));

if (!$form->getWidget('template')->isEmpty())
{
	if ($form->getWidget('code')->isEmpty())
	{
...
if ($form->isPosted())
{
	...
	if(!$form->hasErrors())
	{
			...
			if ($form->isSubmitted())
			{
					return $this->Redirect($id, 'defaultadmin', $returnid, array('tab' => 'templates'));
			}
	}
}
