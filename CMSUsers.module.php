...
class CMSUsers extends CMSModule
{
    ...
    {
        return $this->CheckPermission('Manage CMSUsers');
    }
...
//        $this->loadUser(); // Use the smarty {CMSUser} instead
    ...
    public function SetupRoutes()
    {
        ...
        ;
        $this->RegisterRoute(
            '/user\/validate\/(?P<user_id>[0-9]+)\/(?P<token>[a-zA-Z0-9_-]+)(\/.*?)?$/',
            array(
                'action' => 'validate',
        ...
        $this->RegisterRoute(
            '/user\/show\/(?P<username>[a-zA-Z0-9_-]+)(\/.*?)?$/',
            array(
                'action' => 'user_show',
                'returnid' => $this->GetPreference('default_page', cmsms()->GetContentOperations()->GetDefaultPageID()),
            )
        ...
        $this->RegisterRoute(
            '/user\/(?P<action>[a-zA-Z0-9_-]+)\/(?P<user_id>[0-9]+)\/(?P<token>[a-zA-Z0-9_-]+)(\/.*?)?$/',
            array(
                'returnid' => $this->GetPreference('default_page', cmsms()->GetContentOperations()->GetDefaultPageID())
    ...
    // Function to check all the modules to search for profile module existence
    ...
    public static function getUserOrLogin($params = array()) //$redirect = 'profile', $redirect_url = null)
    {
        ...
        global $id;
        $cmsusers->Redirect($id, 'signin', $returnid, $params);
        return null;
    }
        ...
        switch ($type) {
            ...
            case 'password_reset':
                ...
                $url = $this->CreateLink(
                    ...
                    '',
                    false,
                    'user/password_reset/' . $user->getId() . '/' . $user->getToken() . '/' . $this->GetPreference('default_page', cmsms()->GetContentOperations()->GetDefaultPageID()) . '/'
                );
                break;
        }
...
//        $data .= $this->getHttpAuthValue();
    ...
    private function generateHtAccess()
    {
        ...
        $template_resource = $this->GetFileResource('admin.htaccess.tpl');

        $smarty->assign('auth_name', str_replace(array('"', '\\'), array('', ''), $this->getPreference('htpassword_title', 'Secure area')));
        $smarty->assign('htpasswd_file', $this->getPreference('htpassword_path', $config['root_path']) . DIRECTORY_SEPARATOR . ".htpasswd");

        if ($this->getPreference('ldap_auth', false)) {
            $smarty->assign('ldap_uri', 'ldap://' . $this->getPreference('ldap_server_host') . ':' . $this->getPreference('ldap_server_port') . '/' . $this->getPreference('ldap_base_dn') . '?cn??(objectClass=*)');
            $smarty->assign('ldap_bind_dn', $this->getPreference('ldap_bind_dn'));
            $smarty->assign('ldap_bind_password', $this->getPreference('ldap_bind_password'));
        }

        return $smarty->fetch($template_resource, 'htaccess_cache_id', '');
    }
    ...
    public static function logoutURL()
    {
        ...
        return $protocol . 'logout@' . $domainName;
    }
}